...
class EditLoadCarrierTabPage {
  ...
  private readonly emailNotificationSelect_LOC: Locator;
  private readonly allSelectElements_LOC: Locator;
  private readonly select2ResultOptions_LOC: Locator;
  ...
  constructor(private page: Page) {
    ...
    this.emailNotificationSelect_LOC = page.locator("select#form_notification_address");
    this.allSelectElements_LOC = page.locator("select");
    this.select2ResultOptions_LOC = page.locator("li.select2-results__option");
    this.optionElements_LOC = page.locator("option");
  }
  ...
  async selectEmailNotificationAddress(emailValue: string): Promise<void> {
    if (await this.emailNotificationSelect_LOC.count() > 0) {
      const options = await this.emailNotificationSelect_LOC.locator(this.optionElements_LOC).allTextContents();
      const match = options.find((o: string) => o.toLowerCase().includes(emailValue.toLowerCase()));
      if (match) {
        await this.emailNotificationSelect_LOC.selectOption({ label: match.trim() });
        console.log(`Selected email notification: ${match.trim()}`);
        return;
      }
    }

    // Fallback: scan all selects on the page for the email option
    const allSelects = await this.allSelectElements_LOC.all();
    for (const sel of allSelects) {
      const options = await sel.locator(this.optionElements_LOC).allTextContents();
      const match = options.find((o: string) => o.toLowerCase().includes(emailValue.toLowerCase()));
      if (match) {
    ...
    console.log(`Email notification option not found for: ${emailValue}`);
  }

  /**
   * Selects email notification via the Select2 widget on the Edit Load form.
   * Uses constructor-declared locators. No hardcoded waits.
   * @author AI Agent
   * @created 18-Mar-2026
   */
  async selectEmailNotificationViaSelect2(emailValue: string): Promise<void> {
    await this.emailNotificationSelect2Container_LOC.waitFor({ state: "visible", timeout: WAIT.LARGE });
    await this.emailNotificationSelect2Container_LOC.click();
    await this.emailNotificationSelect2SearchInput_LOC.waitFor({ state: "visible", timeout: WAIT.DEFAULT });
    await this.emailNotificationSelect2SearchInput_LOC.fill(emailValue);
    const resultItem = this.select2ResultOptions_LOC.filter({ hasText: emailValue }).first();
