...
const testCaseID = "BANYAN-74523";
test.describe.configure({ retries: 1 });
test.describe.serial("LTL Imaging - New Customer Portal", { tag: ['@tporegression', '@high'] }, () => {
    ...
    test("Case Id: 74523 - LTL Imaging - New Customer Portal",
        { tag: ['@tporegression', '@banyan', '@p44', '@ltlquote'] }, async () => {
            ...
            await test.step("Search Load in Customer Portal Loads Page", async () => {
                await pages.quoteLTL.searchLoadByID(loadID);
                pages.logger.info(`Searched for Load ID: ${loadID} in Customer Portal Loads Page`);
            });

            await test.step("Verify BOL and Shipping PDF", async () => {
                await pages.quoteLTL.verifyBOLPDF();
                pages.logger.info("BOL PDF verified successfully from Customer Portal");
                await pages.quoteLTL.verifyShippingLabelPDFDownload();
                pages.logger.info("Shipping Label PDF verified successfully from Customer Portal");
            });
        });
});