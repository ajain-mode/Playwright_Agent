...
const testCaseID = "BANYAN-67941-44";
test.describe.configure({ retries: 1 });
test.describe.serial("LTL Quote with Accessorials - Banyan - PCT uplift", { tag: ['@tporegression', '@smoke'] }, () => {
    ...
    test.beforeAll(async ({ browser }) => {
        try {
            ...
            await pages.editCustomerPage.selectPCTRateAndAdjustment(testData.rateTypePCT);
            await pages.editCustomerPage.selectAdjustment(testData.adjustmentValue);
            pages.logger.info("PCT rate type and adjustment selected for customer");
    ...
    test("Case Id: 67941 - LTL Quote with Accessorials - Banyan - Quotes with customer configured rates PCT uplift",
        { tag: ['@tporegression', '@banyan', '@p44', '@ltlquote'] }, async () => {
            pages.logger.info("Starting LTL Quote with Accessorials - Banyan - Quotes with customer configured rates PCT uplift");
    ...
    test("Case Id: 67943 - LTL Quote with Accessorials - Banyan - Create load from quotes with customer rate PCT uplift and e-dispatch",
        { tag: ['@tporegression', '@banyan', '@p44', '@e-dispatch'] }, async () => {
            await test.step("Select tariff and create LTL Quote and verify Booked Status", async () => {
                ...
                await test.step("Profit margin verification on load", async () => {
                    await pages.editLoadLoadTabPage.validateProfitMarginPCT();
                    pages.logger.info("Profit margin verified successfully on load for PCT uplift");
                });
            });
