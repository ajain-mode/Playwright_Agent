...
const testCaseID = "BANYAN-67935-38";
test.describe.configure({ retries: 1 });
test.describe.serial("LTL Quote - Banyan Rating Engine for no customer uplift", { tag: ['@tporegression', '@smoke'] }, () => {
    ...
    test("Case Id: 67935 - LTL Quote - Banyan -Quotes with no customer rates configured",
        { tag: ['@tporegression', '@banyan', '@p44', '@ltlquote'] }, async () => {
            pages.logger.info("Starting LTL Quote - Banyan - Quotes with no customer rates configured");
    ...
    test("Case Id: 67938 - LTL Quote - Banyan - Create load from quotes no customer ratest and e-dispatch",
        { tag: ['@tporegression', '@banyan', '@p44', '@e-dispatch'] }, async () => {
            ...
            await test.step("Verify customer total for NO Cust rate", async () => {
                await pages.editLoadLoadTabPage.verifyCustomerTotalIsZero();
                pages.logger.info("Customer Total verified as $0.00 on load with No Cust Uplift");
            });
