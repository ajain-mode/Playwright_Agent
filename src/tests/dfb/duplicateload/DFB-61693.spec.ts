...
/**
 * Test Case: DFB-61693 - Post a load to the TNX for:- each carrier on a waterfall that has been configured on the load- other non-included carriers after the completion of the waterfall - offer rate on waterfall(Load create through the Tabular form)
 * @author Deepak Bohra
 * @date 02-Dec-2025
 */
 
const testcaseID = "DFB-61693";
...
test.describe
  .serial("Post load to TNX for waterfall carriers and remaining carriers after completion(load created through duplicate method).", () => {
  ...
  test(
    "Case Id: 61693 - Verify TNX load posting for configured waterfall carriers and non-included carriers post waterfall with offer rate on waterfall).",
    {
      tag: "@dfb,@smoke,@tporegression,@offerrateonwaterfall,@manualposting",
    },
    async () => {
      test.setTimeout(1200000); // 15 minutes
