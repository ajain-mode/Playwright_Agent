...
/**
 * Test Case: DFB-61687 - Auto Post a load to the TNX for each carrier on a waterfall  included and non included that has been configured on the load(load created by duplicating the existing load)
 * @author Deepak Bohra
 * @date 25-Nov-2025
 */

const testcaseID = "DFB-61687";
...
test.describe
  .serial("Auto Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through tabular form)", () => {
  ...
  test(
    "Case Id: 61687 -Verify TNX load posting for configured waterfall carriers and non-included carriers post waterfall.",
    {
      tag: "@dfb,@smoke,@tporegression, @waterfall,@autoposting",
    },
    async () => {
