...
/**
 * Test Case: DFB-89166 - Automatically post a load to the TNX for each included carrier on a waterfall. Number of included carriers-3 (load create through non tabular form).
 * @author Parth Rastogi
 * @date 09-Dec-2025
 */

const testcaseID = "DFB-89166";
...
test.describe.serial("Automatically post a load to the TNX for each included carrier on a waterfall. Number of included carriers-3 (load create through non tabular form).", () => {
  ...
  test(
    "Case Id: 89166 - Post a load to the TNX for:- each carrier on a waterfall that has been configured on the load- other non-included carriers after the completion of the waterfall - offer rate on waterfall(Load create through the Non Tabular form).",
    { tag: "@smoke, @dfb,@tporegression,@autoposting, @loadcreation, @waterfall" },
    async () => {
      ...
      await test.step("Open TNX Rep application and perform matching", async () => {
        ...
        try {
          ...
          await tnxRepPages.tnxRepLandingPage.validateRowCountMatchesTotalCarriers(
            dynamicCarrierCount, loadNumber
          );
      ...
      await test.step("Validate the load invisibility for the not included carriers", async () => {
        ...
        const carrierNames = [CARRIER_NAME.CARRIER_4, CARRIER_NAME.CARRIER_5, CARRIER_NAME.CARRIER_7];
       ...
       await test.step("Refresh BTMS page and validate final load status", async () => {
             ...
             await btmsPages.viewLoadCarrierTabPage.clickCancelButton();
             await pages.dfbLoadFormPage.validatePostStatus(LOAD_STATUS.NOT_POSTED);
             await pages.dfbLoadFormPage.validateMixedButtonStates({
               [DFB_Button.Create_Rule]: true,
               [DFB_Button.Post]: true,
             });
             await pages.viewLoadCarrierTabPage.validateCancelButtonNotAvailable();
             await pages.dfbLoadFormPage.hoverOverPostedIcon();
             await pages.dfbLoadFormPage.validateTableFields(sharedPage, {
               "Origin Zip": testData.shipperZip,
               "Destination Zip": testData.consigneeZip,
           ...
           await test.step("Open DME application and perform operations", async () => {
             ...
             try {
               ...
               await dmePages.dmeLoadPage.validateSingleTableRowPresent();
               await dmePages.dmeLoadPage.validateAndGetSourceIdText(loadNumber);
               await dmePages.dmeLoadPage.ValidateDMEStatusText(LOAD_STATUS.BTMS_CANCELLED, LOAD_STATUS.TNX_CANCELLED);
               console.log("✅ DME operations completed successfully");
             } catch (error) {
               console.error("❌ Error in DME operations:", error);
               throw error;
           ...
           await test.step("Open TNX Rep application and perform matching", async () => {
             ...
             const tnxRepPages = await appManager.switchToTNXRep();
              try {
               await tnxRepPages.tnxRepLandingPage.handleOptionalSkipButton();
               await tnxRepPages.tnxRepLandingPage.enterLoadIdFilterValue(loadNumber);
               await tnxRepPages.tnxRepLandingPage.validateNoResultsText();
             } catch (error) {
               console.error("❌ Error in TNX Rep operations:", error);
               throw error;
             }
