...
/**
 * Test Case: DFB-61674 - Auto Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through non  tabular form)
 * @author Parth Rastogi
 * @date 18-Nov-2025
 */

const testcaseID = "DFB-61674";
...
test.describe.serial("Auto Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through non tabular form)", () => {
  ...
  test.afterAll(async () => {
    ...
    if (sharedContext) {
      ...
      test("Case Id: 61674 - Auto Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through non tabular form).",
    { tag: "@smoke, @dfb, @tporegression, @autoposting, @waterfall" },
    async () => {
