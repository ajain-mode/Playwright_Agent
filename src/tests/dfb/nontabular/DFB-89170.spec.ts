...
test.describe("Case ID: 89170 - Enter a bid by a single included carrier on a waterfall for a load that is manally posted to the TNX", () => {
  ...
  test(
    "Case Id: 89170 - Enter a bid by a single included carrier on a waterfall for a load that is manually posted to the TNX",
    ...
    async () => {
      ...
      await test.step("Pre-Conditions setup for creating a load", async () => {
        cargoValue = await pages.dfbHelpers.setupDFBTestPreConditions(
          ...
          testData.officeName,
          toggleSettingsValue,
          pages.toggleSettings.enabled_TNXBids, // ← Now passed as argument
          testData.salesAgent,
      ...
      await test.step("Open TNX Rep application and perform matching", async () => {
        ...
        try {
          ...
          await tnxRepPages.tnxRepLandingPage.clickTnxBiddingButton(TNX.BID_NOW_BUTTON);
          // await tnxRepPages.tnxRepLandingPage.clickTnxBiddingButton(TNX.BID_BUTTON);
          await tnxRepPages.tnxRepLandingPage.enterBidAmount(TNX.BID_RATE);
          await tnxRepPages.tnxRepLandingPage.clickTnxBiddingButton(TNX.BID_NOW_BUTTON);
