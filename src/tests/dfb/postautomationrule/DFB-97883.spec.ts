...
/**
 * Test Case: DFB-97883 - Verify that a new post automation rule is created for a NON EDI customer with include carrier field.
 * @author Deepak Bohra
 * @date 13-Jan-2026
 */

const testcaseID = "DFB-97883";
...
test.describe(
  "Verify that a new post automation rule is created for a NON EDI customer with include carrier field.",
  {
  ...
  () => {
    ...
    test("Case Id: 97883 : Verify that a new post automation rule is created for a NON EDI customer with include carrier field", {
      ...
      await test.step("Validate after edit the post automation rule information is correctly displayed", async () => {
        ...
        await pages.postAutomationRulePageEditEntryModal.validateAllLocationFields({
         ...
         await pages.postAutomationRulePage.getSelectedCarriers(POST_AUTOMATION_RULE.INCLUDE_CARRIER);
