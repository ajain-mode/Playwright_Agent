...
/**
 * Test Case: DFB-97888 - Verify that a new post automation rule is created for a NON EDI customer with Exclude carrier field.
 * @author Deepak Bohra
 * @date 14-Jan-2026
 */

const testcaseID = "DFB-97888";
...
test.describe(
  "Verify that a new post automation rule is created for a NON EDI customer with Exclude carrier field.",
  {
  ...
  () => {
    ...
    test("Case Id: 97888 : Verify that a new post automation rule is created for a NON EDI customer with Exclude carrier field", {
      ...
      await test.step("Add the New post automation rule for a NON EDI customer", async () => {
        ...
        await pages.dfbHelpers.fillPostAutomationRuleForm(
          pages,
          {
            ...
            excludeCarrier: [CARRIER_NAME.CARRIER_2],
          },
          true
        );
      ...
      await test.step("Validate after edit the post automation rule information is correctly displayed", async () => {
        ...
        await pages.postAutomationRulePage.getSelectedCarriers(POST_AUTOMATION_RULE.EXCLUDE_CARRIER);
        await pages.postAutomationRulePage.verifyCarrierSelected(
          CARRIER_NAME.CARRIER_2,
          POST_AUTOMATION_RULE.EXCLUDE_CARRIER
        );
