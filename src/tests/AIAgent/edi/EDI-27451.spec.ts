...
/**
 * Case 27451 — Other stop options (not full replace) + matching location IDs → field update only.
 * @author AI Agent
 * @created 2026-07-17
 */
const testcaseID = "EDI-27451";

test.describe.configure({ retries: 0 });
test.describe(`Case ID: 27451 - EDI Prefs stop field update (matched location IDs)`, () => {
  ...
  test(
    "Case Id: 27451 - Selected stop options update without replacing stops",
    { tag: "@AIAgent,@edi204,@ediprefs" },
    async () => {
      test.setTimeout(WAIT.SPEC_TIMEOUT);

      await test.step("CSV Pre: Configure EDI Prefs — other Stop Details (not full stops replace)", async () => {
      ...
      await test.step("Steps 1–6: Admin Parse original EDI 204", async () => {
        bolNumber = (await dynamicDataAPI.getBolNumber()) + "51";
      ...
      await test.step("Parse change 204 with matching location IDs", async () => {
        await parseEdi204FromFixture(
          pages,
          changeFixtureForScenario("matchLocUpdate"),
          bolNumber,
          testData.sender204ID || "PSKL",
          testData.senderAsID || "AVGD",
        );
      ...
      await test.step("Step 10 [Expected]: Stops not replaced (same shipper name)", async () => {
