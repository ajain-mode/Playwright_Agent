...
test.describe("Case ID: 171225 - Post automation rule match include carriers", () => {
  ...
  test(
    "Case Id: 171225 - Post automation rule match include carriers",
    { tag: "@AIAgent,@dfb,@loads" },
    async () => {
      test.setTimeout(WAIT.SPEC_TIMEOUT_LARGE); // 15 minutes — DME status sync can exceed 10m
      ...
      await test.step("Enter offer rate, save, and validate DFB form state in view mode", async () => {
        ...
        // Re-baseline after Post — Post can also bump BIDS; Match Now must increase from this value
        bidsReportValue =
          await pages.viewLoadCarrierTabPage.getBidsReportValue();
