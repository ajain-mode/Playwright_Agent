...
test.describe.serial(
  "Case ID: DFB-97739 - Automatically book a load when it is manually posted (carrier auto accept with missing contact validation)",
  () => {
    ...
    test(
      ...
      async () => {
        ...
        await test.step("Step 6: Switch to DME and verify carrier is enabled with toggle ON (Precondition Steps 36-40)", async () => {
          ...
          const toggleState = await dmePages.getCarrierToggleState(testData.Carrier);
          pages.logger.info(`DME carrier toggle: ${toggleState.enabled ? "ON" : "OFF"}`);

          if (!toggleState.enabled && toggleState.found) {
            await dmePages.enableCarrierToggle(testData.Carrier);
