    ...
    test(
      ...
      async () => {
        ...
        await test.step("Step 31: Click VIEW BILLING and validate Payable Toggle, carrier over-invoiced message, and Carrier payable status", async () => {
          ...
          const actualMsg = await pages.loadBillingPage.findPayableMessageContaining(FINANCE_MESSAGES.CARRIER_OVER_INVOICED);
          pages.logger.info(`Actual payable message: ${actualMsg}`);
          expect(
            actualMsg,
