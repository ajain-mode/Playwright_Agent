...
export class CodeGenerator {
  ...
  // Removed: _matchScore (was only used by LLM code path)
  ...
  private pomMatcher: POMMethodMatcher;
  private _processedSteps: ProcessedStep[] = [];
  ...
  constructor(config?: AgentConfig, llmService?: LLMService) {
    ...
    this.pomMatcher = new POMMethodMatcher(
      this.schemaAnalyzer.getPageObjectScanner(),
      null,
    );
  }

  getProcessedSteps(): ProcessedStep[] {
    return this._processedSteps;
  }

  getPOMMethodMatcher(): POMMethodMatcher {
    return this.pomMatcher;
  }

  setAppSourceIndexer(indexer: import('../analyzers/AppSourceIndexer').AppSourceIndexer): void {
    this.pomMatcher = new POMMethodMatcher(
      this.schemaAnalyzer.getPageObjectScanner(),
      indexer,
    );
  ...
  async generateScript(testCase: TestCaseInput, testData?: TestData, dynamicRefSpecPath?: string, matchScore?: number): Promise<GeneratedScript> {
    this._processedSteps = [];
    this.pomMatcher.resetSession();
  ...
  private ensurePageObjectMethodsExist(content: string, testCase: TestCaseInput): string {
    ...
    while ((match = pageCallRegex.exec(content)) !== null) {
      ...
      if (this.isGenericUtilityMethod(method, locator)) {
        ...
        const commonReusablesPath = path.resolve(this.config.projectRoot, 'src/utils/commonReusables.ts');
...
await this.page.waitForLoadState('load');`,
  ...
  private async generateCodeFromAction(action: string, _testData?: TestData, preProcessed?: ProcessedStep): Promise<string> {
    ...
    if (mapping && mapping.confidence >= 0.8) {
      const captures = action.toLowerCase().match(mapping.pattern);
      const code = resolveCodePlaceholders(mapping.code, captures, _testData);
      console.log(`      🗺️ Matched mapping [${mapping.category}]: ${mapping.pageObject}.${mapping.method}`);
      return code;
    }
    ...
    if (matchResult.type === 'existing-pom' || matchResult.type === 'mapping') {
      ...
      console.log(`      🆕 New POM method proposed: ${matchResult.reason}`);
    } else {
  ...
  private validatePostGenerationGuardrails(code: string, testCase: TestCaseInput, testData?: TestData): string {
    ...
    // All other validation steps → expect.soft() (soft, default)
    {
      ...
      if (hardAssertionStepKeywords.length > 0) {
        ...
        fixed = fixed.replace(stepBlockRegex, (_fullMatch, stepHeader: string, stepBody: string, stepClose: string) => {
          ...
          if (isHardAssertionStep) {
            ...
            const converted = stepBody.replace(/(?<!\.)expect\(/g, () => {
              hardToSoftCount++;
              return 'expect.soft(';
            });
            return stepHeader + converted + stepClose;
