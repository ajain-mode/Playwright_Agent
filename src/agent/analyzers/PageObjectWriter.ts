...
export class PageObjectWriter {
  ...
  private insertMethods(
    ...
    for (const method of methods) {
      // Validate: warn if method body contains inline locators
      const inlineWarnings = this.validateNoInlineLocators(method.name, method.body);
      ...
      // JSDoc comment — always ensure @author and @created tags are present
      if (method.jsdocComment) {
        let jsdoc = method.jsdocComment;
        // Enforce mandatory @author tag
        if (!/@author\s/.test(jsdoc)) {
          jsdoc = jsdoc.replace(/\*\//, '* @author AI Agent\n   */');
        }
        // Enforce mandatory @created tag
        if (!/@created\s/.test(jsdoc)) {
          const today = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-');
          jsdoc = jsdoc.replace(/\*\//, `* @created ${today}\n   */`);
        }
        methodStr += `  ${jsdoc}\n`;
      } else {
        const today = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-');
        ...
        methodStr += `   * @author AI Agent\n`;
        methodStr += `   * @created ${today}\n`;
        methodStr += `   */\n`;
      }
  ...
  /**
   * Validate that a method body does not contain inline locator definitions.
   * Locators must be initialized in the constructor and referenced via this.<name>_LOC.
   * Logs warnings for inline locators found in method bodies.
   */
  private validateNoInlineLocators(methodName: string, body: string): string[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (inlineLocatorPattern.test(lines[i])) {
        warnings.push(
          `   ⚠️ POM Standard Violation: Method '${methodName}' has inline locator at body line ${i + 1}. ` +
          `Move locator to constructor as a private readonly property.`
        );
      }
