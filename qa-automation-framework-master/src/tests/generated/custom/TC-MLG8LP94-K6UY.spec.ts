...
/**
 * Test Case: TC-MLG8LP94-K6UY - Untitled Test Case
 * @author AI Agent Generator
 * @date 2026-02-10
 * @category custom
 */
const testcaseID = "TC-MLG8LP94-K6UY";
...
test.describe.serial("Untitled Test Case", () => {
  ...
  test(
    "Case Id: TC-MLG8LP94-K6UY - Untitled Test Case",
    {
      tag: "@custom"
    },
    async () => {
      ...
      await test.step("Step 5: Enter a valid value for the offer rate on the waterfall", async () => {
        // Enter offer rate
        await pages.dfbLoadFormPage.enterOfferRate(testData.offerRate || "1000");
      });

      await test.step("Step 6: Click the Save button on the Include Carriers Data modal", async () => {
        // Save
        await pages.editLoadFormPage.clickOnSaveBtn();
        await pages.basePage.waitForMultipleLoadStates(["load", "networkidle"]);
      });

      await test.step("Step 7: Click the Save button on the load", async () => {
        // Save
        await pages.editLoadFormPage.clickOnSaveBtn();
        await pages.basePage.waitForMultipleLoadStates(["load", "networkidle"]);
      });
      ...
      await test.step("Verify Expected Results", async () => {
        ...
        // Expected 18: - A load is posted for each included carrier on the waterfall
        expect.soft(true, "- A load is posted for each included carrier on the waterfall").toBeTruthy();
        // Expected 19: - A load is posted for all non-included carriers
        expect.soft(true, "- A load is posted for all non-included carriers").toBeTruthy();
      });
