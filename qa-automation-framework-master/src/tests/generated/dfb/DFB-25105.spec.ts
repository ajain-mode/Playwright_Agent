...
/**
 * Test Case: DFB-25105 - Verify that a message is displayed when a value is not entered for the Email for Notifications field on the CREATE NEW ENTRY form
 * @author AI Agent Generator
 * @date 2026-02-17
 * @category dfb
 */
const testcaseID = "DFB-25105";
...
test.describe.serial("Verify that a message is displayed when a value is not entered for the Email for Notifications field on the CREATE NEW ENTRY form", () => {
  ...
  test(
    "Case Id: DFB-25105 - Verify that a message is displayed when a value is not entered for the Email for Notifications field on the CREATE NEW ENTRY form",
    {
    ...
    async () => {
      ...
      await test.step("Verify Expected Results", async () => {
        // Expected 1: A message is displayed relating 'Invalid target rate supplied'
        // Verify message displayed using existing commonReusables.validateAlert
        await Promise.all([
          pages.commonReusables.validateAlert(sharedPage, ALERT_PATTERNS.INVALID_TARGET_RATE_SUPPLIED),
