  ...
  test(
    ...
    async () => {
      ...
      await test.step("Step 1: Login BTMS", async () => {
        ...
        pages.logger.info(" I am running my code here ");
      ...
      await test.step("Step 5: Select/Enter valid values for all of the fields on the CR...", async () => {
        ...
        await dfbHelpers.fillPostAutomationRuleForm(pages,
          {
      ...
      await test.step("Step 6: Click Create and Verify Expected Alert Message", async () => {
        // Click Create triggers a browser alert() dialog for missing email
        // verifyAlertMessage handles: register dialog listener → click → capture → assert
        await pages.basePage.verifyAlertMessage(
