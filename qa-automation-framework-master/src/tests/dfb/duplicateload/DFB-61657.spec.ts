...
test.describe("Post a load to the TNX that is only visible to carriers selected for the Include Carriers field on the load(load create by duplicating the load.", () => {
  ...
  test(
    "Case Id: 61657 - Validate the include carrier present on the load is displaying the load created through Duplicating the load.",
    { tag: "@dfb,@smoke,@tporegression,@includecarrier,@manualposting" },
    async () => {
