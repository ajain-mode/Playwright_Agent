...
/**
 * Test Case: DFB-61669 - Post a load to the TNX that is only visible to carriers selected for the Include Carriers field on the load(load create by duplicating the existing load).
 * @author Parth Rastogi
 * @date 12-Nov-2025
 */

const testcaseID = "DFB-61669";
...
test.describe.serial("Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create by duplicating the existing load)", () => {
  ...
  test(
    "Case Id: 61669 - Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create by duplicating the existing load)",
    { tag: "@smoke, @dfb, @tporegression, @waterfall, @manualposting" },
    async () => {
