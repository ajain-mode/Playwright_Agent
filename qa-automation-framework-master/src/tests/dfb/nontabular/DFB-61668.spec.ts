...
/**
 * Test Case: DFB-61668 - Post a load to the TNX that is only visible to carriers selected for the Include Carriers field on the load(load create through non tabular form).
 * @author Parth Rastogi
 * @date 12-Nov-2025
 */

const testcaseID = "DFB-61668";
...
test.describe.serial("Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through non tabular form)", () => {
  ...
  test(
    "Case Id: 61668 - Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through non tabular form).",
    { tag: "@smoke, @dfb ,@manualposting, @tporegression, @waterfall" },
    async () => {
