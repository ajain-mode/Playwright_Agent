...
const getLoadData = () => ({
  ...
  shipperNameUS: testData.shipperNameUS,
  shipperNameCA: testData.shipperNameCA,
  shipperNameMX: testData.shipperNameMX,
...
test.describe('Case ID: 25101 - Individual Field Validation Tests for Non-Tabular Load',{tag: ['@smoke','@dfb', '@tporegression','@fieldvalidation']}, () => {
  ...
  test('Validate Invalid Shipper ZIP/Postal Code with all mandatory fields filled for US, Canada, and Mexico Locations', async () => {
    ...
    for (const country of countries) {
      const validationResult = await pages.nonTabularLoadPage.validateShipperInvalidZipCodeWithMandatoryFields(
        country.code,
        loadData
      );
