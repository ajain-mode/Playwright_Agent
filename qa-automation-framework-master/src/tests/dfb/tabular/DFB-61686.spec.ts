...
/**
 * Test Case: DFB-61674 - Auto Post a load to the TNX for each carrier on a waterfall carrier included and non included that has been configured on the load(load create through non tabular form)
 * @author Deepak Bohra
 * @date 22-Nov-2025
 */

const testcaseID = "DFB-61686";
...
test.describe.serial("Auto Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through non tabular form)", () => {
  ...
  test.afterAll(async () => {
    ...
    if (sharedContext) {
      ...
      test("Case Id: 61686 - Auto Post a load to the TNX for each carrier on a waterfall that has been configured on the load(load create through non tabular form).",
    { tag: "@dfb,@loadposting,@tporegression,@waterfall,@smoke,@autoposting" },
    async () => {
