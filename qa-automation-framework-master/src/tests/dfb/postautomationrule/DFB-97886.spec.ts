...
/**
 * Test Case: DFB-97886 - Verify that a new post automation rule is created for a NON EDI customer with "Post to all Carriers upon completion of the Waterfall" is checked and offer rate on waterfall.
 * @author Deepak Bohra
 * @date 19-Jan-2026
 */
 
const testcaseID = "DFB-97886";
...
test.describe(
  "Verify that a new post automation rule is created for a NON EDI customer.",
  {
  ...
  () => {
    ...
    test("Case Id: 97886 : Verify that a new post‑automation rule is created for a non‑EDI customer with Post to all carriers upon completion of the waterfall enabled and Offer Rate on Waterfall selected", {
      ...
      // Define carriersData at test level scope so it can be accessed by multiple test steps
      const carriersData = [
        {
      ...
      await test.step("Add the New post automation rule for a NON EDI customer", async () => {
        ...
        await pages.postAutomationRulePage.clickViewDetailsLink();
        ...
        await pages.postAutomationRulePage.clickCloseIncludeCarrierDetailsModal();
      ...
      await test.step("Validate the new post automation rule get added on the Post Automation Rule page", async () => {
        ...
        // Verify input values for carriers
        await pages.postAutomationRulePage.clickViewDetailsLink();
        await pages.dfbIncludeCarriersDataModalWaterfall.verifyCarrierInputValues(carriersData);
        await pages.dfbIncludeCarriersDataModalWaterfall.validatePostAllCarrierCheckboxIsChecked();
        await pages.postAutomationRulePageEditEntryModal.validateWaterfallOfferRate(TNX.OFFER_RATE_3);
      });
    });
  }
);