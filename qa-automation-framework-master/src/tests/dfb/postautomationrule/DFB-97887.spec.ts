...
/**
 * Test Case: DFB-97887 - Verify that a new post automation rule is created for a NON EDI customer with for carrier auto accept .
 * @author Parth Rastogi
 * @date 15-Jan-2026
 */

const testcaseID = "DFB-97887";
...
test.describe(
  "Verify that a new post automation rule is created for a NON EDI customer with for carrier auto accept field.",
  {
  ...
  () => {
    ...
    test(
      "Case Id: 97887 : Verify that a new post automation rule is created for a NON EDI customer with for carrier auto accept field",
      {
      ...
      async () => {
        ...
        await test.step("Add the New post automation rule for a NON EDI customer", async () => {
          ...
          await pages.dfbHelpers.fillPostAutomationRuleForm(
            ...
            false, // addStop1
            true, // clickCarrierAutoAccept - Click on carrier auto accept checkbox
            "deepak.bohra@modeglobal.com"  // carrierAcceptAsUserEmail
          );
        ...
        await test.step("Validate after edit the post automation rule information is correctly displayed", async () => {
          ...
          // Verify carrier auto accept checkbox is checked
          const isCarrierAutoAcceptChecked =
            await pages.postAutomationRulePage.isCarrierAutoAcceptCheckboxChecked();
          console.log(
            `Carrier auto accept checkbox is ${
              isCarrierAutoAcceptChecked ? "checked" : "unchecked"
            }`
          );

          // Verify carrier accept as user email is selected
          await pages.postAutomationRulePage.verifyCarrierAcceptAsUserSelected(
            "deepak.bohra@modeglobal.com"
          );
