...
test.describe(
  "Create a load  through template and post it to TNX and validate the load status in DME and TNX applications.",
  {
  ...
  () => {
    ...
    test("Case Id: 61630 : Create a load through template and validate DME and TNX", async () => {
      ...
      await test.step("Setup DFB Test Environment", async () => {
        cargoValue = await dfbHelpers.setupDFBTestPreConditions(
          ...
          LOAD_TYPES.CREATE_TL_NEW, // <-- keep this as requested
          false, // skipPostAutomationRule
          true,
          skipNavigateToLoad
        );
      });
      ...
      await test.step("Post a Load manually through a template", async () => {
        ...
        await pages.loadTemplateSearchPage.selectTemplateRecord(
          ...
          //LOAD_TEMPLATE_SEARCH_PAGE.TEMPLATE_VALUE_8
        ...
        pages.logger.info("Click on Save button and Create Load button");
        ...
        await test.step("Open TNX application and perform matching", async () => {
          ...
          await appManager.tnxPage.setViewportSize({
            width: 1920,
            height: 1080,
          });
          try {
        ...
        await test.step("Refresh BTMS page and validate final load status", async () => {
          ...
          await btmsPages.dfbLoadFormPage.validatePostStatus(
            LOAD_STATUS.MATCHED
          );
