...
/**
 * Test Case: DFB-61702 - Post a load to the TNX for:- each carrier on a waterfall that has been configured on the load- other non-included carriers after the completion of the waterfall - offer rate on waterfall(load create through Template)
 * @author Parth Rastogi
 * @date 24-Nov-2025
 */

const testcaseID = 'DFB-61702';
...
test.describe('Case ID: 61702 - Post a load to the TNX for:- each carrier on a waterfall that has been configured on the load- other non-included carriers after the completion of the waterfall - offer rate on waterfall(load create through Template)', { tag: ['@smoke', '@dfb', '@tporegression','@autoposting', '@offerrateonwaterfall'] }, () => {
  ...
  test('Post a load to the TNX for:- each carrier on a waterfall that has been configured on the load- other non-included carriers after the completion of the waterfall - offer rate on waterfall(load create through Template)', async () => {
    ...
    await test.step('Post a Load manually through a template', async () => {
      ...
      await test.step("Enter offer rate, save, and validate DFB form state in view mode", async () => {
        ...
        await pages.dfbLoadFormPage.enterOfferRate(TNX.OFFER_RATE_3);
