...
/**
 * Test Case to Auto Post a load to the TNX (load created by Template) and check include carrier present on the load is displaying the load created through EDI.#
 * @author Deepak Bohra
 * @date 28-Nov-2025
 */
const testcaseID = "DFB-61690";
...
test.describe
  ...
  test(
    "Case Id: 61690 - Load is visible to carriers in Include Carriers; for non-included carriers, it’s visible with offer rate entered on load.",
    {
      ...
      await test.step("Post a Load manually through a template", async () => {
        ...
        await test.step("Enter offer rate, save, and validate DFB form state in view mode", async () => {
          ...
          await test.step("Update multiple carriers in loop", async () => {
            ...
            console.log(
              ` Dynamic carrier count set to: ${dynamicCarrierCount}`
            );
