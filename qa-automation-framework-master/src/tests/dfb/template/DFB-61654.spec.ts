...
test.describe(
  "Case ID: 61654 - Enter a Bid for a Load posted manually to the TNX - 2(bid made by agent in btms) and load created by Template",
  { tag: ["@smoke", "@dfb", "@tporegression", "@manualposting", "@bidding"] },
  () => {
    ...
    test("Enter a Bid for a Load posted manually to the TNX(load create through Template)", async () => {
      ...
      await test.step("Enter offer rate, save, and validate DFB form state in view mode", async () => {
        ...
        await pages.viewLoadPage.createBTMSBidForLoad(
          CARRIER_NAME.CARRIER_6,
          testData.bidAmount,
          TNX.BID_SUCCESS_MESSAGE
        );
      ...
      await test.step("Open DME application and perform operations", async () => {
        ...
        try {
          ...
          await dmePages.dmeLoadPage.verifyDMELoadDetailsLabel(
            TNX.CARRIER_NAME
          );
          await dmePages.dmeLoadPage.verifyDMELoadDetailsLabel(
            testData.bidRate
          );
          //await dmePages.dmeLoadPage.verifyDMELoadDetailsLabel(LOAD_STATUS.PENDING);
