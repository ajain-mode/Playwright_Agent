...
const testCaseID = "BANYAN-74510";
test.describe.configure({ retries: 2 });
test.describe("LTL Tracking - BTMS - Manually Rated Loads", { tag: ['@tporegression', '@smoke'] }, () => {
    ...
    test("Case Id: 74510 - LTL Tracking - BTMS - Manually Rated Loads",
        { tag: ['@tporegression', '@banyan', '@p44', '@ltlquote'] }, async () => {
            pages.logger.info("Starting LTL Tracking - BTMS - Manually Rated Loads as load is already created in beforeAll hook with booked status");

            await test.step("Go to admin and open simulator tool", async () => {
                await pages.adminPage.hoverAndClickAdminMenu();
                await pages.adminPage.clickOnSimulatorToolLink();
                pages.logger.info("Navigated to Simulator Tool page successfully");
            });

            await test.step("Enter form details and submit", async () => {
                await pages.simulateEDispatchPage.selectSourceFromDropdown(testData.simulatedSource);
                await pages.simulateEDispatchPage.enterBTMSLoadID(loadId);
                await pages.simulateEDispatchPage.selectEventDropdown(testData.simulatedEvent);
                await pages.simulateEDispatchPage.enterLocationDetails
                    (testData.simulatedCity,
                        testData.simulatedState,
                        testData.simulatedZip);
                await pages.simulateEDispatchPage.clickOnSubmitButton();
                pages.logger.info("Simulator Tool form submitted successfully");
            });
            ...
            await test.step("Go to Tracking and verify event", async () => {
                await pages.viewLoadPage.clickOnTrackingTab();
                await pages.viewLoadPage.verifyEventDetailsInTable(
                    testData.simulatedEvent,
                    testData.simulatedCity,
                    testData.simulatedState,
                    testData.simulatedZip
                );
