...
const testCaseID = "BANYAN-67951";
test.describe.configure({ retries: 1 });
test.describe.serial("Change carrier by updating load with LTL quoted rates", { tag: ['@tporegression', '@smoke'] }, () => {
    ...
    test("Case Id: 67951 - Change carrier by updating load with LTL quoted rates",
        { tag: ['@banyan', '@p44', '@ltlquote', '@tporegression'] }, async () => {
            ...
            await test.step("Click on Save and Edit and dispatch load and verification of new banyanQuoteId and customer total value", async () => {
                await pages.editLoadFormPage.clickOnSaveBtn();

                const initialValue = await pages.viewLoadPage.getCustomerTotalValue();
                pages.logger.info(`Initial Customer Total Value: ${initialValue}`);

                await pages.editLoadCarrierTabPage.clickOnCarrierTab();

                const initialBanyanQuoteId = await pages.viewLoadCarrierTabPage.banyanQuoteId();
                pages.logger.info(`Initial Banyan Quote ID: ${initialBanyanQuoteId}`);

                await pages.viewLoadCarrierTabPage.clickOnChangeCarrier();
                await pages.ltlQuoteRequestPage.verifyTariffTable();
                pages.logger.info("Tariff table verified successfully after clicking Change Carrier");
                await pages.ltlQuoteRequestPage.chooseCarrier();
                pages.logger.info("New carrier chosen successfully from tariff table after clicking Change Carrier");

                const newValue = await pages.viewLoadPage.getCustomerTotalValue();
                pages.logger.info(`New Customer Total Value: ${newValue}`);
                expect(newValue).not.toBe(initialValue);
                pages.logger.info("Customer total value changed successfully after changing carrier.");

                await pages.editLoadCarrierTabPage.clickOnCarrierTab();
                const newBanyanQuoteId = await pages.viewLoadCarrierTabPage.banyanQuoteId();
                pages.logger.info(`New Banyan Quote ID: ${newBanyanQuoteId}`);
                expect(newBanyanQuoteId).not.toBe(initialBanyanQuoteId);
                pages.logger.info("Banyan Quote ID changed successfully after changing carrier.");
